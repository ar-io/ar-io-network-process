...
local function addRecordResultFields(ioEvent, result)
	...
	ioEvent:addFieldsIfExist(
		result.record,
		{ "startTimestamp", "endTimestamp", "undernameLimit", "purchasePrice", "primaryNameAddress" }
	)
...
Handlers.add(ActionMap.ClaimPrimaryName, utils.hasMatchingTag("Action", ActionMap.ClaimPrimaryName), function(msg)
	local checkAssertions = function()
		assert(type(msg.Tags.Name) == "string", "Invalid name")
	end
...
Handlers.add(ActionMap.MatchPrimaryName, utils.hasMatchingTag("Action", ActionMap.MatchPrimaryName), function(msg)
	local function checkAssertions()
...
Handlers.add(ActionMap.RemovePrimaryName, utils.hasMatchingTag("Action", ActionMap.RemovePrimaryName), function(msg)
...
Handlers.add(ActionMap.PrimaryName, utils.hasMatchingTag("Action", ActionMap.PrimaryName), function(msg)
	local primaryName = arns.getPrimaryName(msg.Tags.From)
...
Handlers.add(ActionMap.PrimaryNames, utils.hasMatchingTag("Action", ActionMap.PrimaryNames), function(msg)
...
-- TODO: FIX THIS!
Handlers.add("paginatedPrimaryNames", utils.hasMatchingTag("Action", "Paginated-Primary-Names"), function(msg)
	local page = utils.parsePaginationTags(msg)
	local status, result =
	...
	if not status then
		ao.send({
			Target = msg.From,
			Action = "Invalid-Paginated-Primary-Names-Notice",
			Error = "Pagination-Error",
			Data = json.encode(result),
		})
	else
