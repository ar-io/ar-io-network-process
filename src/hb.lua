...
if not getmetatable(PrimaryNames.requests) then
	PrimaryNames.requests = listen.addListener(PrimaryNames.requests, function(ctx)
		_G.HyperbeamSync.primaryNames.requests[ctx.key] = true
	end)
end
...
function hb.createPrimaryNamesPatch()
	...
	-- Note: PrimaryNames tables are proxies, so we need to check the actual data
	local namesActualData = listen.getActualData(PrimaryNames.names)
	local ownersActualData = listen.getActualData(PrimaryNames.owners)
	local requestsActualData = listen.getActualData(PrimaryNames.requests)

	if next(namesActualData) == nil then
	...
	if next(ownersActualData) == nil then
	...
	if next(requestsActualData) == nil then
