...
describe("epochs", function()
	...
	describe("getRewardRateForEpoch", function()
		...
		it("returns a linearly decreasing rate starting from 0.1% after 365 epochs, up to 5 decimal places", function()
			local rate = epochs.getRewardRateForEpoch(366)
			assert.are.equal(0.001, rate)
			local rateHalfwayThroughPeriod = epochs.getRewardRateForEpoch(366 + 91)
			assert.are.equal(0.00075, rateHalfwayThroughPeriod)
			local rateAtEndOfPeriod = epochs.getRewardRateForEpoch(366 + 182)
			assert.are.equal(0.0005, rateAtEndOfPeriod)
		end)
