...
describe('GatewayRegistry', async () => {
    ...
    async function decreaseDelegateStakeTest({
      ...
      if (instant) {
        ...
        assert.deepStrictEqual(decreaseDelegateStakeResult.Messages[0], {
          ...
          Anchor: '00000000000000000000000000000012',
          Tags: [
            {
              name: 'Action',
  ...
  describe('Paginated-Gateways', () => {
    ...
    it('should filter gateways by nested boolean field with false value', async () => {
      // add a gateway with autoStake set to false
      const fourthGatewayAddress = 'fourth-gateway-'.padEnd(43, 'c');
      const noAutoStakeTags = [
        { name: 'Action', value: 'Join-Network' },
        { name: 'Label', value: 'test-gateway-4' },
        { name: 'Note', value: 'test-note' },
        { name: 'FQDN', value: 'no-autostake.example.com' },
        { name: 'Operator-Stake', value: `${INITIAL_OPERATOR_STAKE}` },
        { name: 'Port', value: '443' },
        { name: 'Protocol', value: 'https' },
        { name: 'Allow-Delegated-Staking', value: 'true' },
        { name: 'Min-Delegated-Stake', value: `${INITIAL_DELEGATE_STAKE}` },
        { name: 'Delegate-Reward-Share-Ratio', value: '30' },
        { name: 'Observer-Address', value: fourthGatewayAddress },
        {
          name: 'Properties',
          value: 'FH1aVetOoulPGqgYukj0VE0wIhDy90WiQoV3U2PeY44',
        },
        { name: 'Auto-Stake', value: 'false' }, // explicitly set to false
      ];
      const { memory: addGatewayMemory4 } = await joinNetwork({
        address: fourthGatewayAddress,
        memory: sharedMemory,
        timestamp: STUB_TIMESTAMP + 3,
        tags: noAutoStakeTags,
      });

      // Test filtering by autoStake=false
      const paginatedGatewaysFalse = await handle({
        options: {
          Tags: [
            { name: 'Action', value: 'Paginated-Gateways' },
            {
              name: 'Filters',
              value: JSON.stringify({ 'settings.autoStake': false }),
            },
          ],
        },
        memory: addGatewayMemory4,
        timestamp: STUB_TIMESTAMP + 3,
      });

      const falseResult = JSON.parse(
        paginatedGatewaysFalse.Messages?.[0]?.Data,
      );
      // should find the gateway with autoStake=false
      assert.equal(falseResult.totalItems, 1);
      assert.equal(falseResult.items.length, 1);
      assert.equal(falseResult.items[0].gatewayAddress, fourthGatewayAddress);
      assert.equal(falseResult.items[0].settings.autoStake, false);

      // Test filtering by autoStake=true (should exclude the false one)
      const paginatedGatewaysTrue = await handle({
        options: {
          Tags: [
            { name: 'Action', value: 'Paginated-Gateways' },
            {
              name: 'Filters',
              value: JSON.stringify({ 'settings.autoStake': true }),
            },
          ],
        },
        memory: addGatewayMemory4,
      ...
      const trueResult = JSON.parse(paginatedGatewaysTrue.Messages?.[0]?.Data);
      // should find the other gateways with autoStake=true (default)
      // The exact number depends on test setup, but should be at least 1
      assert.ok(
        trueResult.totalItems >= 1,
        `Should find at least 1 gateway with autoStake=true, got ${trueResult.totalItems}`,
      );
      assert.equal(trueResult.items.length, trueResult.totalItems);
      // verify our gateway with autoStake=false is not in the results
      assert.ok(
        trueResult.items.every(
          (g) => g.gatewayAddress !== fourthGatewayAddress,
        ),
        'Gateway with autoStake=false should not appear in autoStake=true filter',
      );

      sharedMemory = addGatewayMemory4;
    });
  });
  ...
  describe('Save-Observations', () => {
    ...
    it('should fail to save an observation after before the epoch has started', async () => {
      const { result } = await saveObservations({
        from: observerAddress,
        failedGateways,
        reportTxId,
    ...
    it('should fail to save an observation after the epoch has ended', async () => {
      const { result } = await saveObservations({
        from: observerAddress,
        failedGateways,
        reportTxId,
